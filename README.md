...
## 🔄 CI/CD Pipeline (Point 10)

The project includes a **GitHub Actions CI pipeline** (`.github/workflows/ci.yml`) that runs automatically on pushes to `main`/`develop` and on pull requests.

### Pipeline Stages

| Stage | Description | Tools |
|-------|-------------|-------|
| **Backend** | Install dependencies → Run `pytest` suite | Python 3.11, pip cache |
| **Frontend** | Install dependencies → Run `jest` → Build production bundle | Node 20, npm ci |
| **Docker Build** | Build both images → Start compose → Health check `/health` | Docker Compose |

The Docker build stage only runs after both test suites pass, ensuring broken code never reaches a container image.
...
### Completed Scope
*   **Complete RESTful API**: Implemented auth endpoints (`/login`, `/refresh`), participant CRUD endpoints (`/participants`), and metrics aggregation endpoint (`/metrics`) with Pydantic type validation and error handling.
*   **Live Metrics Dashboard**: The dashboard KPI cards (Total Enrolled, Retention Rate, Study Arms, Demographics) are dynamically computed from live database records via the `/api/v1/metrics` endpoint using SQL aggregate functions.
